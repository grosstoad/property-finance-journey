...
export const usePropTrack = () => {
  ...
  const [propertyAttributes, setPropertyAttributes] = useState<PropertyAttributes | null>(null);
  ...
  const getPropertyDetails = useCallback(async (propertyId: string) => {
    try {
      ...
      try {
        ...
        const valuationParams: Record<string, any> = {};
        
        if (attributes?.attributes) {
          const attrs = attributes.attributes;
          
          if (attrs.bedrooms) valuationParams.bedrooms = attrs.bedrooms.value;
          if (attrs.bathrooms) valuationParams.bathrooms = attrs.bathrooms.value;
          if (attrs.carSpaces) valuationParams.carSpaces = attrs.carSpaces.value;
          if (attrs.landArea) valuationParams.landArea = attrs.landArea.value;
          if (attrs.floorArea) valuationParams.floorArea = attrs.floorArea.value;
          if (attrs.propertyType) valuationParams.propertyType = attrs.propertyType.value;
        }
